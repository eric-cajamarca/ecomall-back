...
const obtener_reviews_producto_publico = async function(req,res){
    ...
    let reviews = await Review.find({producto:id, estado:'Activo'}).populate('cliente').sort({createdAt:-1});
...
const obtener_cliente_admin = async function(req,res){
    ...
    if(req.user){
            ...
            try{
                //aqui capturo si hay un error en el id
                var reg = await Cliente.findById({_id:id});
...
module.exports = {
    ...
    listar_productos_categoria_publico
}